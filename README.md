...
- **誦讀**：直式（豎排）閱讀全文，可調字級；放大或縮小時會依可視高度重新排版，放不下的字自動接到左側下一直行。向左滑動翻閱，呼應傳統經本的閱讀方向。桌機亦可用滾輪或 ←→ 方向鍵翻閱；最右可見行（目前讀到的行）以朱砂標示。
...
之後每次更新程式或經文，記得把 `sw.js` 開頭的快取版本號往上加。目前版本是 `CACHE = "chisong-v4"`；下一次更新改成 v5，已安裝的使用者重開頁面後才會換到新版快取。
...
iOS 27 Safari 可能將 `writing-mode: vertical-rl` 的百分比高度算得比實際可視區域更高，造成每一直行底部的字被排到框外後裁掉。本專案不再只依賴 CSS 百分比高度，而是讀取捲動框的實際 `clientHeight`，以像素設定直排行高；字級、螢幕方向或 Safari 工具列高度改變時會重新計算，超出的字會接續到左側下一直行。

網站更新後若仍看到舊版排版，請將 Safari 分頁或主畫面 App 完全關閉再重開；Service Worker 會換用新版離線快取。
